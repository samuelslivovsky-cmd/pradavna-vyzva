...
export function renderEmail({ eyebrow, title, lines = [], list = [], cta, photo }) {
  const photoHtml =
    photo && SITE
      ? `<div style="margin:6px 0 26px;"><img src="${SITE}/sehe.jpg" alt="Sehe" width="128" height="128" style="width:128px;height:128px;border-radius:64px;object-fit:cover;border:3px solid ${C.gold};display:inline-block;" /></div>`
      : ''
  ...
  // Tmavé pozadie cez obrázok — dark-mode klienti (iOS Gmail) obrázky
  // NEprefarbujú, takže tma ostane. bgcolor je fallback, keď sa obrázok nenačíta.
  const BG = SITE ? `${SITE}/email-bg.png?v=2` : ''
  const CARD = SITE ? `${SITE}/email-card.png?v=2` : ''
  const outerBgAttr = SITE ? ` background="${BG}"` : ''
  const outerBgCss = SITE
    ? `background-image:url('${BG}');background-repeat:repeat;background-size:cover;background-position:top center;`
    : ''
  const cardBgAttr = SITE ? ` background="${CARD}"` : ''
  const cardBgCss = SITE
    ? `background-image:url('${CARD}');background-repeat:repeat;background-size:cover;background-position:top center;`
    : ''
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="${C.bg}"${outerBgAttr} style="${outerBgCss}background-color:${C.bg};">
<tr><td align="center" bgcolor="${C.bg}"${outerBgAttr} style="${outerBgCss}background-color:${C.bg};padding:48px 16px;">
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="max-width:520px;width:100%;">
<tr><td bgcolor="${C.card}"${cardBgAttr} style="${cardBgCss}background-color:${C.card};border:1px solid ${C.border};border-radius:18px;padding:52px 30px;text-align:center;font-family:Georgia,'Times New Roman',serif;color:${C.text};">
...
export function sigilEmail(hint) {
  return {
    subject: '☾ Padla nová pečať, Sehe',
    text: `${hint.title}\n\nNová pečať čaká, Sehe. Otvor Pradávnu výzvu a odhaľ, čo ti šepká.${SITE ? `\n\n${SITE}` : ''}`,
    html: renderEmail({
      eyebrow: 'Padla nová pečať',
      title: hint.title,
      photo: true,
      lines: [
        'Nová pečať čaká, <strong>Sehe</strong>.',
...
export function departureEmail() {
  return {
    subject: '🎒 Dnes sa vyráža, Sehe',
    text: `Dobrodružstvo začína. Dúfam, že máš zbalené to správne…${SITE ? `\n\n${SITE}` : ''}`,
    html: renderEmail({
      eyebrow: 'Deň D',
      title: 'Dobrodružstvo začína',
      photo: true,
      lines: ['Dnes sa vyráža, <strong>Sehe</strong>.', 'Dúfam, že máš zbalené to správne…'],
      cta: openCta(),
    }),
  }
}
...
export function solvedEmail({ id, title, answer, attempts }) {
  ...
  return {
    subject: `✅ Sehe uhádol — ${answer ?? ''}`,
    text: `Sehe správne vyplnil pečať.\n\nPečať: ${title}\nOdpoveď: ${answer}\nUhádol na: ${pokus}`,
    html: renderEmail({
      eyebrow: 'Sehe uhádol pečať',
      title: title ?? 'Pečať',
...
export function proofEmail({ title, text, hasPhoto }) {
  const lines = []
  if (text) lines.push(text)
  if (hasPhoto) {
    lines.push(
      `<img src="cid:proof" alt="dôkaz" style="max-width:100%;border-radius:12px;border:1px solid ${C.border};margin-top:8px;" />`,
    )
  }
  return {
    subject: `📸 Sehe splnil úlohu — ${title ?? ''}`,
    text:
      `Sehe splnil úlohu.\n\nÚloha: ${title ?? '?'}\n` +
      `Dôkaz: ${text || '(bez textu)'}${hasPhoto ? '\n(priložená fotka)' : ''}`,
    html: renderEmail({
      eyebrow: 'Sehe splnil úlohu',
      title: title ?? 'Úloha',
      lines,
      cta: openCta(),
    }),
