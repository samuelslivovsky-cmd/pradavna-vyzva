...
export function renderEmail({ eyebrow, title, lines = [], list = [], cta, photo }) {
  ...
  const outerBgCss = SITE ? `background-image:url('${SITE}/email-bg.png');background-repeat:repeat;` : ''
  const cardBgAttr = SITE ? ` background="${SITE}/email-card.png"` : ''
  const cardBgCss = SITE ? `background-image:url('${SITE}/email-card.png');background-repeat:repeat;` : ''
